...
.glueTrees = function(ttree,wtree)  {
  ...
  n <- nrow(ttree)
  nh <- n-nsam
  no <- nsam + 1 # starting index of internal nodes
  ma<-0
  ## biggest number of offsprings
  for (i in (1:n)) {ma<-max(ma,nrow(wtree[[i]])/2)} 
  lab<-matrix(0,n,ma)
  labh<-rep(0,n)
  for (i in (n:1)) {#Consider all hosts 
    ni <- nrow(wtree[[i]])/2 # number of internal nodes
    for (j in (1:ni)) { 
      lab[i,j] <- no 
      no <- no + 1 
    } 
    labh[i] <- no-1 # last index of a within-host tree
  } 
  leaves <- cbind() 
  intnodes <- cbind() 
  for (i in (1:n)) { 
    ...
    if (!is.na(ttree[i,2])) leaves <- rbind(leaves,tree[1, ]) #If sampled, the case is the first row
