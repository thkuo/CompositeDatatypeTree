...
#' @return unglued ttree and w
#' @export
simulateOutbreak_unglued = function(off.r=1,off.p=0.5,neg=0.25,nSampled=NA,pi=0.5,w.shape=2,w.scale=1,ws.shape=NA,ws.scale=NA,w.mean=NA,w.std=NA,ws.mean=NA,ws.std=NA,dateStartOutbreak=2000,dateT=Inf) {
  ...
  ttree[,1]<-ttree[,1]+dateStartOutbreak
  ttree[,2]<-ttree[,2]+dateStartOutbreak
  l=list(ttree= ttree, wtrees= wtree,nam=mtt$nam,probttree=probttree,probwithin=probwithin)
  return(l)
}  
