...
#+++++
# Core functions
def make_alf_config(config_template, project_name, genome_db_f, tree_f,
                    rates_f, output_dir):
    ...
    with open(config_template, 'r') as config_template_h:
        with open(config_file, 'w') as config_fh:
            for l in config_template_h:
                ...
                elif re.search('\$RATES_FILE\$', l):
                    l= re.sub('\$RATES_FILE\$', rates_f, l)
                config_fh.write(l)
    return(config_file)

def make_alf_rates(genome_db_f, project_name, output_dir):
    r= 1.57e-6 # substitutions per site per year
    # read the inital sequences
    sequences= []
    for l in open(genome_db_f,'r').readlines():
        check_pattern= re.search('<DNA>(.+)</DNA>', l)
        if not (check_pattern is None):
            sequences.append(check_pattern.group(1))
    seq_ls= [len(s) for s in sequences]
    rates= [l*r for l in seq_ls]
    rates_f= os.path.join(output_dir, '{}_rates.txt'.format(project_name))
    with open(rates_f, 'w') as rates_fh:
        rates_fh.write(str(rates))
    return(rates_f)
...
alf_rates_file= make_alf_rates(genome_db_f, project_name, output_dir)
alf_config_file= make_alf_config(config_template, project_name, genome_db_f,
                                 tree_f, alf_rates_file, output_dir)
# run alf
