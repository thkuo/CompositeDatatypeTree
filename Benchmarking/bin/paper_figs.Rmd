...
for (c in cutoffs){
    col_nuc_tr_f<- sprintf('../results/ll_cutoff/%s/raxml_input/nuc_col.nwk', 
                         as.character(c))
    stopifnot(file.exists(col_nuc_tr_f))
    cd_tr_f<- sprintf('../results/ll_cutoff/%s/RAxML_bestTree.conc',
                    as.character(c))
    stopifnot(file.exists(cd_tr_f))
...
for (tr.name1 in c('col25', 'col50', 'col75', 'col100')){
    ...
    violin_p<- ggplot(sub_cophe_one_df, aes(x= loc_pair))+
        geom_violin(aes(y= cd_dist-nuc_dist, fill= transmission))+
    ...
    cophenetic_plots[[tr.name1]]<- list('violin'= violin_p, 'bars'= bars_p)
}
cophenetic_plots
