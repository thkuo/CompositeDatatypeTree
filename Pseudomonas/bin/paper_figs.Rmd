...
# visualize the positions of sampling locations 
cd_loc_p<-  ggtree(trs$cd75, layout = 'circular', branch.length = 'none')+
    geom_tippoint(aes(color= traits[label]), size= 1, alpha= .6)+
    geom_nodepoint(aes(subset= node %in% unique_ixs$cd75), color= 'red', size= 2, shape= 18)+
    scale_color_manual(values= c('Charité Berlin'= 'gold',
                                 'University hospital Frankfurt'= 'yellowgreen',
                                 'Hannover Medical School'='blue'), name= 'hospital')+
    ggtitle('composite datatype')+
    theme(legend.position= 'bottom')
nuc_loc_p<-  ggtree(trs$nuc, layout = 'circular', branch.length = 'none')+
    geom_tippoint(aes(color= traits[label]), size= 1, alpha= .6)+
    geom_nodepoint(aes(subset= node %in% unique_ixs$nuc), color= 'red', size= 2, shape= 18)+
    scale_color_manual(values= c('Charité Berlin'= 'gold',
                                 'University hospital Frankfurt'= 'yellowgreen',
                                 'Hannover Medical School'='blue'), name= 'hospital')+
    ggtitle('nucleotide')+
    theme(legend.position= 'bottom')
ggarrange(plotlist = list(cd75=cd_loc_p, nuc=nuc_loc_p), nrow=1, common.legend = T, legend = 'bottom')
# visualize the positions of sampling locations 
load_mlst<- function(){
    mlst_f<- '../data/sample2seqtype.txt'
    mlst_df<- read.csv(mlst_f, header= T, sep= '\t',
                       quote = '"' , check.names = F,
                       stringsAsFactors = F)  
    mlst_df$group_id<- as.character(mlst_df$group_id)
    mlst_df$group_id[grep('[^0-9]', mlst_df$group_id)]<-NA
    mlst_df
}
mlst_df<- load_mlst()
mlst_dict<- mlst_df[match(trs$nuc$tip.label, mlst_df[, 'Isolate']), 'group_id']
names(mlst_dict)<- trs$nuc$tip.label
mlst_counts<- table(mlst_dict)
target_mlsts<- names(mlst_counts[mlst_counts >= 5])
mlst_dict<- mlst_dict[mlst_dict %in% target_mlsts]
mlst_col_dict<- brewer.pal(n=length(target_mlsts), name='Set3')
names(mlst_col_dict)<- target_mlsts
cd_mlst_p<-  ggtree(trs$cd75, layout = 'circular', branch.length = 'none')+
    geom_tippoint(aes(color= mlst_dict[label]), size= 1, alpha= .6)+
    geom_nodepoint(aes(subset= node %in% cd_new_ixs), color= 'red', size= 2, shape= 18)+
    scale_color_manual(values = mlst_col_dict, name= 'serotype', breaks = target_mlsts)+
    ggtitle('composite datatype')+
    theme(legend.position= 'bottom')
nuc_mlst_p<-  ggtree(trs$nuc, layout = 'circular', branch.length = 'none')+
    geom_tippoint(aes(color= mlst_dict[label]), size= 1, alpha= .6)+
    geom_nodepoint(aes(subset= node %in% nuc_new_ixs), color= 'red', size= 2, shape= 18)+
    scale_color_manual(values = mlst_col_dict, name= 'serotype', breaks = target_mlsts)+
    ggtitle('nucleotide')+
    theme(legend.position= 'bottom')
ggarrange(plotlist = list(cd75=cd_mlst_p, nuc=nuc_mlst_p), nrow=1, common.legend = T, legend = 'bottom')
```
...
Count the cophenetic distances
```{R}
norm_tr<- function(tr){
...
sub_cophe_one_df$transmission<- ifelse(sub_cophe_one_df$loc1 == sub_cophe_one_df$loc2, 'within-hospital', 'across-hospital')
ggplot(sub_cophe_one_df, aes(x= loc_pair))+
    geom_violin(aes(y= cd75_dist-nuc_dist, fill= transmission))+
    theme_classic()+
